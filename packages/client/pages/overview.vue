...
const { openInEditor, slides, hasGrid } = useNav()

// Group slides by their grid column for the sidebar mini-map
...
<template>
  <div class="h-screen w-screen of-hidden flex">
        ...
        <div class="absolute left-0 top-0 bottom-0 w-200 flex flex-col flex-auto items-end group p2 gap-1 max-h-full of-x-visible of-y-auto" style="direction:rtl">
          <template v-if="hasGrid">
            <div
              ...
              style="direction:ltr"
            >
              <div class="text-2xs op30 text-center font-mono mb0.5">{{ colIdx + 1 }}</div>
              <div
                v-for="route of col"
                :key="route.no"
                class="relative"
    ...
    <!-- Unified main content: CSS grid placement in grid mode, flat list otherwise -->
    <main class="flex-1 h-full of-auto" @scroll="checkActiveBlocks">
      <div
        :class="hasGrid ? 'inline-grid' : ''"
        :style="hasGrid ? `grid-template-columns: repeat(${gridColumns.length}, auto)` : ''"
      >
        <div
          v-for="(route, idx) of slides"
          :key="route.no"
          :ref="el => blocks.set(idx, el as HTMLElement)"
          :style="hasGrid ? { gridColumn: (route.meta.slide?.gridCol ?? 0) + 1, gridRow: (route.meta.slide?.gridRow ?? 0) + 1 } : undefined"
          class="relative of-hidden flex gap-4 min-h-50 group"
          :class="[
            idx === 0 ? 'pt5' : '',
            hasGrid ? 'border-b border-main' : 'border-t border-main',
            hasGrid && (route.meta.slide?.gridCol ?? 0) < gridColumns.length - 1 ? 'border-r border-main' : '',
            hasGrid && (route.meta.slide?.gridRow ?? 0) === 0 ? 'border-t-2 border-t-primary/30' : '',
          ]"
        >
          <div class="select-none w-13 text-right my4 flex flex-col gap-1 items-end">
            ...
            <div v-if="hasGrid" class="text-xs op30 font-mono mb1">
